...
int do_action(struct run_act_ctx* h, struct action* a, struct sip_msg* msg)
{
	int ret = 0;
	int v = 0;
	struct dest_info dst = {0};
	char* tmp = NULL;
	char *new_uri = NULL , *end=NULL, *crt=NULL;
	ksr_cmd_export_t* cmd = NULL;
	int len = 0;
	int user = 0;
	struct sip_uri uri={0}, next_hop={0};
	struct sip_uri *u={0};
	unsigned short port = 0;
	str* dst_host = NULL;
	int i = 0, flags = 0;
	avp_t* avp = NULL;
	struct search_state st ={0};
	struct switch_cond_table* sct =NULL;
	struct switch_jmp_table*  sjt =NULL;
	struct rval_expr* rve = NULL;
	struct match_cond_table* mct = NULL;
	struct rvalue* rv = NULL;
	struct rvalue* rv1 = NULL;
	struct rval_cache c1 ={0};
	str s;
	void *srevp[2];
